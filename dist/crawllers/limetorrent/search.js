...
async function search(search, site = LIMETORRENT_SITE) {
  try {
    ...
    await browser.userAgent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.87 Safari/537.36");
    ...
    var searchResults = await page.evaluate(async () => {
      ...
      tableRows.forEach(item => {
        if (!item.querySelector("th")) {
          var details = "Added: " + item.querySelector("td:nth-of-type(2)").innerText + ", Size: " + item.querySelector("td:nth-of-type(3)").innerText;
          results.push({
    ...
    return {
      error: true,
      errorMessage: "Runtime error occured"
    };
  }
}
